...
<section>
    <div class="container my-5">
        ...
        <div class="row">
            <%
                List<Product> featuredProducts = (List<Product>) request.getAttribute("featured-products");
                for (Product product : featuredProducts) { %>

            <div class="col-lg-3 col-md-6 col-sm-6 d-flex">
                <div class="card w-100 my-2 shadow-2-strong">
                    <img src="<%= request.getContextPath() +"/img/user-images/" + product.getProductImagePath() %>"
                         class="card-img-top p-2" style="aspect-ratio: 1 / 1"/>
                    <div class="card-body d-flex flex-column">
                        <h5 class="card-title"><%= product.getProductName() %></h5>
                        <p class="card-text">$<%= product.getProductPrice() %></p>
                        <div class="card-footer d-flex align-items-end pt-3 px-0 pb-0 mt-auto">
                            <a onclick="addItemToCart(<% out.print(product.getId()); %>); updateButtonStyle('addItem-<% out.print(product.getId()); %>');" id="addItem-<% out.print(product.getId()); %>" class="btn btn-primary shadow-0 me-1">Add to cart</a>
                        </div>
                    </div>
                </div>
            </div>
